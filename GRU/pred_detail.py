...
# ---- Dosya yolları ----
MODEL_PATH = 'GRU_balanced_model_v3.keras'
...
# ---- Model yükleme ----
console.log("[bold cyan]GRU Model yükleniyor (derlemeden)...[/]")
model = load_model(
    MODEL_PATH,
...
console.log("[bold green]GRU Model yüklendi![/]")
...
# ---- Ön işleme ve tahmin ----
console.log("[bold cyan]GRU ile tahmin işlemi başlıyor...[/]")
...
main_table = Table(title="🎯 GRU Detaylı Tahmin Sonuçları", expand=True, box=box.ROUNDED)
...
console.print("\n")
prob_table = Table(title="📊 GRU - Tüm Sınıflar İçin Olasılık Dağılımı", expand=True, box=box.DOUBLE_EDGE)
...
stats_table = Table(title="📈 GRU - Sınıf Bazlı İstatistiksel Analiz", expand=True)
...
console.print(Panel(ozet_text, title="🎯 GRU Analiz Özeti", border_style="green"))

# ---- Alternatif Tahminler ----
console.print("\n")
alt_table = Table(title="🔄 GRU - İkinci ve Üçüncü En Yüksek Olasılıklar", expand=True)
...
print("\n" + "="*80)
print("✅ GRU modeli ile detaylı analiz tamamlandı!")
print("💡 Bu analiz size GRU modelinin her tahmini için güven seviyesi, alternatif tahminler ve")
