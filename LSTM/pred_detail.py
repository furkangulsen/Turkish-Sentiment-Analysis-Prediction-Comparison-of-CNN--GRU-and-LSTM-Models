...
# ---- Dosya yolları ----
MODEL_PATH = 'lstm_balanced_model_v3.keras'
TOKENIZER_PATH = 'tokenizer_balanced.pickle'
LABELENCODER_PATH = 'labelencoder_balanced.pickle'
...
# ---- Model yükleme ----
console.log("[bold cyan]LSTM Model yükleniyor (derlemeden)...[/]")
model = load_model(
    MODEL_PATH,
...
console.log("[bold green]LSTM Model yüklendi![/]")
...
# ---- Ön işleme ve tahmin ----
console.log("[bold cyan]LSTM ile tahmin işlemi başlıyor...[/]")
...
main_table = Table(title="🎯 LSTM Detaylı Tahmin Sonuçları", expand=True, box=box.ROUNDED)
...
console.print("\n")
prob_table = Table(title="📊 LSTM - Tüm Sınıflar İçin Olasılık Dağılımı", expand=True, box=box.DOUBLE_EDGE)
...
stats_table = Table(title="📈 LSTM - Sınıf Bazlı İstatistiksel Analiz", expand=True)
...
console.print(Panel(ozet_text, title="🎯 LSTM Analiz Özeti", border_style="green"))

# ---- Alternatif Tahminler ----
console.print("\n")
alt_table = Table(title="🔄 LSTM - İkinci ve Üçüncü En Yüksek Olasılıklar", expand=True)
...
print("\n" + "="*80)
print("✅ LSTM modeli ile detaylı analiz tamamlandı!")
print("💡 Bu analiz size LSTM modelinin her tahmini için güven seviyesi, alternatif tahminler ve")
